...
async function sendResend(to: string, subject: string, html: string) {
  // Central delivery gate: nothing leaves while email_delivery_enabled is false.
  const gateAdmin = createClient(
    Deno.env.get("SUPABASE_URL") ?? "",
    Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "",
  );
  const result = await sendGatedEmail(gateAdmin, {
    from: "The Diabetes Reset Method <hello@diabetesresetmethod.com>",
    to,
    subject,
    html,
  });
  if (!result.sent) console.warn("magic link email suppressed or failed:", result.reason);
}
