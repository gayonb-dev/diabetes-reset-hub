...
## Approved clarifications (binding)

- The full original prompt remains the authority; this file summarises it. V01–V16 are executed and reported individually with their original subrequirements.
- Screenshot authenticity means the real routes rendered in the isolated local/preview environment. Before the browser fixture runs, all backend requests are intercepted — Auth, database and Edge Functions, including `create-subscription-checkout` — not just stripe.com. Unexpected requests to production services are blocked and fail the run. Evidence is labelled isolated browser verification, never live-backend verification. No production identities, auth-setting changes or CORS/RLS loosening.
- Water precision (verified: `public.water_logs.ounces` is `integer`, i.e. whole US fl oz): schema and historical logs unchanged; the entered quantity stays stable across unit switches with no repeated rounding; storage rounding is applied once at submission; when the saved amount differs from the entry the exact amount to be saved is shown first (250 mL → saves 8 fl oz ≈ 237 mL); an entry rounding to zero is rejected with a clear message and creates no log or award; the reloaded total matches the stored quantity; "Saved" appears only after persistence succeeds.
- Shared consumers: every consumer of the header/CTA components is inspected so no page outside `Index` crashes on a missing checkout provider or loses working navigation; `src/lib/units.ts` gains volume helpers without altering existing weight/glucose exports. Focused regressions cover legal-page headers and the sticky-CTA/VITA interaction only.
